...
def ensemble_file_system_operation_set(
    ...
    return EnsembleFileSysOperationSet(
        [
            ensemble_copy_operation,
            ensemble_symlink_operation,
            ensemble_configure_operation,
        ]
    )


def test_init_ensemble_copy_operation(mock_src: pathlib.Path, mock_dest: pathlib.Path):
...
def test_init_ensemble_symlink_operation(mock_src: str, mock_dest: str):
...
def test_init_ensemble_configure_operation(mock_src: str):
    """Validate EnsembleConfigureOperation init"""
    ensemble_configure_operation = EnsembleConfigureOperation(
        mock_src, file_parameters={"FOO": ["BAR", "TOE"]}
    )
...
def test_init_ensemble_file_sys_operation_set(
    ...
    ensemble_fs_op_set = EnsembleFileSysOperationSet(
...
def test_add_ensemble_copy_operation(
...
def test_add_ensemble_symlink_operation(
    ensemble_file_system_operation_set: EnsembleFileSysOperationSet,
):
    ...
    assert (
        len(ensemble_file_system_operation_set.symlink_operations) == orig_num_ops + 1
    )
...
def test_add_ensemble_configure_operation(
    ...
    ensemble_file_system_operation_set.add_configuration(
        src=pathlib.Path("/src"), file_parameters={"FOO": "BAR"}
    )
    assert (
        len(ensemble_file_system_operation_set.configure_operations) == orig_num_ops + 1
    )
...
def test_ensemble_configure_files_invalid_dest(dest, error, source):
    ...
    with pytest.raises(error):
        _ = [
            EnsembleConfigureOperation(
                src=file, dest=dest, file_parameters={"FOO": "BAR"}
            )
            for file in source
        ]
