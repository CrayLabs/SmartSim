...
def test_create_scheduler_settings(scheduler_enum):
    bs_str = BatchSettings(batch_scheduler=scheduler_enum.value, scheduler_args={"launch":"var"}, env_vars={"ENV":"VAR"})
    assert bs_str._batch_scheduler == scheduler_enum
    # TODO need to test scheduler_args
    assert bs_str._env_vars == {"ENV":"VAR"}
    
    bs_enum = BatchSettings(batch_scheduler=scheduler_enum, scheduler_args={"launch":"var"}, env_vars={"ENV":"VAR"})
