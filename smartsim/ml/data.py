...
class BatchDownloader:
    ...
    By default, the BatchDownloader to be created in a process
    launched through SmartSim, with producers listed as incoming
    entities. All batches are downloaded 
    ...
    To set up a `BatchDownlaoder`, after creation, the user has to call
    `init_sources()` and `init_samples()`.  After `init_sources()` is called,
    a list of data sources is populated, representing the batches which
    will be downloaded. Each source is represented as a tuple `(producer_name, sub_index)`.
    Before `init_samples()` is called, the user can also modify the list.
