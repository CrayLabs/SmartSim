          		    ...
          		    T_recv comp_value, std::string key_suffix="")
{
  ...
  if(!client.poll_key(key.c_str(),200,10))
    throw std::runtime_error("Poll for key failed.");
...
int main(int argc, char* argv[]) {
  ...
  check_scalar<double>(&SmartSimClient::poll_key_and_check_scalar_double,
		                   std::numeric_limits<double>::max(), "_dbl");

  check_scalar<float>(&SmartSimClient::poll_key_and_check_scalar_float,
                      std::numeric_limits<float>::max(), "_flt");

  check_scalar<int64_t>(&SmartSimClient::poll_key_and_check_scalar_int64,
		                    std::numeric_limits<int64_t>::min(), "_i64");

  check_scalar<int32_t>(&SmartSimClient::poll_key_and_check_scalar_int32,
		                    std::numeric_limits<int32_t>::min(), "_i32");

  check_scalar<uint64_t>(&SmartSimClient::poll_key_and_check_scalar_uint64,
	                       std::numeric_limits<uint64_t>::max(),"_ui64");

  check_scalar<uint32_t>(&SmartSimClient::poll_key_and_check_scalar_uint32,
