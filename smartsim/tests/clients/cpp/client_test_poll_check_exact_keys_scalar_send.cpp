...
int main(int argc, char* argv[]) {
  ...
  put_scalar<double>(&SmartSimClient::put_exact_key_scalar_double,
    std::numeric_limits<double>::max(), "_dbl");

  put_scalar<float>(&SmartSimClient::put_exact_key_scalar_float,
    std::numeric_limits<float>::max(), "_flt");

  put_scalar<int64_t>(&SmartSimClient::put_exact_key_scalar_int64,
		std::numeric_limits<int64_t>::min(), "_i64");

  put_scalar<int32_t>(&SmartSimClient::put_exact_key_scalar_int32,
		std::numeric_limits<int32_t>::min(), "_i32");

  put_scalar<uint64_t>(&SmartSimClient::put_exact_key_scalar_uint64,
		std::numeric_limits<uint64_t>::max(), "_ui64");

  put_scalar<uint32_t>(&SmartSimClient::put_exact_key_scalar_uint32,
