...
void test_1d_put_cpp(int dim1, std::string key_suffix="")
{
  ...
  client.put_1d_array_double(key.c_str(), array, dim1);
  client.get_1d_array_double(key.c_str(), result, dim1);
...
void test_2d_put_cpp(int dim1, int dim2, std::string key_suffix="")
{
  ...
  client.put_2d_array_double(key.c_str(), arr, dim1, dim2);

  double** result = (double **)malloc(dim1 * sizeof(double *));
  for(int i=0; i<dim1; i++)
    result[i] = (double *)malloc(dim2 * sizeof(double));

  client.get_2d_array_double(key.c_str(), result, dim1, dim2, 0, 0);
...
void test_3d_put_cpp(int dim1, int dim2, int dim3, std::string key_suffix="")
{
  ...
  client.put_3d_array_double(key.c_str(), arr, dim1, dim2, dim3);
  ...
  client.get_3d_array_double(key.c_str(), result, dim1, dim2, dim3);
...
void test_nd3_put_cpp(int dim1, int dim2, int dim3, std::string key_suffix="")
{
  SmartSimClient client;
  int rank;
  int n_ranks;
  ...
  if(rank == 0)
    std::cout<<"Finished 3d put/get with nD array c++ test"<<std::endl;
...
int main(int argc, char* argv[]) {
  ...
  test_nd3_put_cpp(atoi(argv[1]), atoi(argv[1]), atoi(argv[1]));
  MPI_Finalize();
