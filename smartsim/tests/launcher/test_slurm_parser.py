...
def test_parse_salloc():
    output = "salloc: Granted job allocation 118568\n" \
...
def test_parse_salloc_extra():
    output = "salloc: Running node verification test prior to job execution.\n"\
...
def test_parse_salloc_high():
    output = "salloc: Granted job allocation 29917893\n" \
             "salloc: Waiting for resource configuration\n" \
...
def test_parse_salloc_error_2():
    output = "salloc: unrecognized option '--no-a-option'\n"\
...
def test_parse_salloc_error_4():
    output = "salloc: error: No hardware architecture specified (-C)!\n" \
...
def test_parse_sacct_step_id():
    output = "SmartSim|119225|\n" \
             "extern|119225.extern|\n" \
             "m1-119225.0|119225.0|\n" \
             "m2-119225.1|119225.1|"
    step_id = "119225.0"
    parsed_step_id = slurmParser.parse_step_id_from_sacct(output,
                                                          "m1-119225.0")
...
def test_parse_sacct_step_id_2():
    ...
    step_id = "119225.2"
    parsed_step_id = slurmParser.parse_step_id_from_sacct(output,
                                                          "orchestrator_0-119225.2")
    assert(step_id == parsed_step_id)

def test_parse_sacct_step_id_2():
    output = "SmartSim|962333|\n" \
